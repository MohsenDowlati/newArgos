...
export default function Metrics() {
  ...
  async function getCameradata() {
    ...
    const PersonDirectionDetails = Object.keys(groupedDataPerson).map(
      (key) => ({ [key]: groupedDataPerson[key] })
    )
    ...
    const CarDirectionDetails = Object.keys(groupedDataCar).map((key) => ({
      [key]: groupedDataCar[key],
    }))
    ...
    const BikeDirectionDetail = Object.keys(groupedBikeData).map((key) => ({
      [key]: groupedBikeData[key],
    }))
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div className="rounded-xl bg-red-500 p-5 text-white">
  ...
  return (
    <div className="w-full bg-[#212326] ">
      ...
      <div className="flex h-fit">
        ...
        <div className="min-h-screen  w-full bg-[#292c30]">
          <div className="flex w-full  ">
            <FilterBar
              ApplyFunction={getCameradata}
              SETstartdate={setStartdate}
              setCamera_id={setCamera_id}
              SETenddate={setEnddate}
              startdate={startdate}
              enddate={enddate}
            />
          </div>
          <p className="mt-5 ml-10 text-white">
            - Results are based on selected filter{' '}
          </p>
          <div className=" items-center  pb-10">
            ...
            {isloaded ? (
              <CarChart
                timelist={CarTimelist}
                data={CarDirection}
                detialData={CarDetails}
                camera_id={camera_id}
                start_date={startdate}
                end_date={enddate}
              />
            ...
            {isloaded ? (
              <BikeChart
                timelist={BikeTimelist}
                data={BikeDirection}
                detialData={BikeDetail}
                camera_id={camera_id}
                start_date={startdate}
                end_date={enddate}
              />
